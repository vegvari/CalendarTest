...
class CalendarTest extends PHPUnit_Framework_TestCase
{
    ...
    public function testCalendar(
        ...
        $this->assertSame(
            $day,
            $calendar->getDay(),
            'Asserting that the day is being returned correctly'
        );

        $this->assertSame(
            $weekday,
            $calendar->getWeekDay(),
            'Asserting that the week is being returned correctly'
        );

        $this->assertSame(
            $first_weekday,
            $calendar->getFirstWeekDay(),
            'Asserting that the first week day is being returned correctly'
        );

        $this->assertSame(
            $number_of_days,
            $calendar->getNumberOfDaysInThisMonth(),
            'Asserting that the number of days in this month is being returned correctly'
        );

        $this->assertSame(
            $number_of_days_prev,
            $calendar->getNumberOfDaysInPreviousMonth(),
            'Asserting that the number of days in the previous month is being returned correctly'
        );

        $this->assertSame(
            $first_week,
            $calendar->getFirstWeek(),
            'Asserting that the first week in this month is being returned correctly'
        );

        $this->assertSame(
            $result,
            $calendar->getCalendar(),
            'Asserting that the calendar is being drawn correctly'
        );
    }
